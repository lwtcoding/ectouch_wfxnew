...
class wxpay
{
     ...
     * @return bool
     */
    public function transfers($obj, $payment){
        ...
        try {
            ...
            $this->parameters["mch_appid"] = $this->payment['wxpay_appid']; // 公众账号ID
            $this->parameters["mch_id"] = $this->payment['wxpay_mchid']; // 商户号
            $this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR']; // 终端ip
            $this->parameters["nonce_str"] = $this->createNoncestr(); // 随机字符串
            $this->parameters["partner_trade_no"] = $this->get_order_sn(); // 获取新订单号
            $this->parameters["check_name"] = "NO_CHECK"; // 获取新订单号
            $this->parameters["amount"] = $obj["amount"];
            $this->parameters["desc"] = '用户提现';
        ...
        $response = $this->postXmlCurlWithCert($xml,$url, 30,ROOT_PATH.$payment['wxpay_cert_path'], ROOT_PATH.$payment['wxpay_key_path']);
        ...
        if (isset($result["result_code"]) && $result["result_code"] == 'SUCCESS') {
            ...
            return [
                'code'=>400,
                'msg'=>$result['return_msg'].(isset($result['err_code_des']) ? $result['err_code_des'] : '')
            ];
        }
        $prepay_id = $result["return_code"];
        //返回是否成功
        return $prepay_id;
    }
    /**
     ...
     * 作用：以post方式提交xml到对应的接口url
     */
    public function postXmlCurlWithCert($xml, $url, $second = 30, $cert_path ='', $key_path ='')
    {
        // 初始化curl
        $ch = curl_init();
        // 设置超时
        ...
        if ($data) {
            ...
            $error = curl_errno($ch);
            $e_msg = curl_error($ch);
            echo "curl出错，错误码:$error,错误信息：" .$e_msg. "<br>";
            curl_close($ch);
            return false;
        }
    }
