...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        $up_uid     = get_affiliate();
        if ($up_uid) {
            $is_proxy = $global->db->getOne("SELECT is_proxy FROM " . $global->ecs->table('users') . " WHERE user_id = '$up_uid'");
            if ($is_proxy){
                //设置推荐人
                $sql = 'UPDATE '. $global->ecs->table('users') . ' SET parent_proxy_user_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
