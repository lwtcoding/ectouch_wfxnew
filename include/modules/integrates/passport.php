...
".$this->field_name." <> '".$cfg['post_username']."'";
    ...
    public function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop)) {
            ...
            if ($col) {
                $sql = "UPDATE {pre}users SET parent_id = 0,parent_proxy_user_id = 0  WHERE " . db_create_in($col, 'parent_id'); // 将删除用户的下级的parent_id 改为0
                $this->db->query($sql);
    ...
    { 
        if (empty($username)) {
            ...
            $sql = 'SELECT user_id, password, email, is_proxy, is_vip FROM {pre}users ' . " WHERE user_name='$username' LIMIT 1";
            $row = $this->db->queryRow($sql);
            ...
            if ($row) {
                ...
                $_SESSION['is_proxy'] = $row['is_proxy'];
                $_SESSION['is_vip'] = $row['is_vip'];
            }
        }
    }
