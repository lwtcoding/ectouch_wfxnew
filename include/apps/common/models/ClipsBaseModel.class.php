...
class ClipsBaseModel extends BaseModel {
    ...
    public function get_user_default($user_id) {
        ...
        $sql = "SELECT pay_points, user_money, credit_line,parent_id, last_login, is_validated, is_proxy, is_vip FROM " . $this->pre . "users WHERE user_id = '$user_id'";
        $row = $this->row($sql);		
        $info = array();
		if($row['parent_id'] > 0){
        ...
        $info['is_proxy'] = $row['is_proxy'];
        $info['is_vip'] = $row['is_vip'];
        //新增获取用户头像，昵称
        $u_row = '';
