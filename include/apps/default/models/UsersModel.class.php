...
class UsersModel extends BaseModel {
    ...
    function update_user_info() {
        ...
        /* 查询会员信息 */
        $time = date('Y-m-d');
        $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, u.parent_proxy_user_id, u.is_proxy, ' .
        ...
        if ($row = $this->row($sql)) {
            //如果登录了并且上级代理为0或则parent0设为默认代理的id
            //找总代理
            $sql = "select * from ". $this->pre ."admin_user where  proxy_user = 1 and proxy_default = 1 ";
            $total_proxy =$this->row($sql);
            //判断是否代理帐号
            $sql = "select * from ". $this->pre ."admin_user where  proxy_user_id =  ".$_SESSION['user_id'];
            $proxy_user =$this->row($sql);
            if ($total_proxy && !$row['parent_proxy_user_id'] && !$proxy_user) {
                $sql = "update ". $this->pre ."users set parent_proxy_user_id=".$total_proxy['proxy_user_id']." , parent_id=".$total_proxy['proxy_user_id']." where  user_id = ".$_SESSION['user_id'];
                $this->query($sql);
            }

            header('location: '.url('store/index',['u'=>$drp['user_id'],'drp_id'=>$drp['id']]));
            /* 更新SESSION */
    ...
    function register($username, $password, $email, $other = array()) {
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
            ...
            //定义other合法的变量数组
            $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'parent_id', 'aite_id', 'nick_name', 'identity', 'address', 'country', 'province', 'city','districts');
...
//                }
//            }
            //获取推广人，如果是代理则保存，不是则置为0
            $up_uid = model('Users')->get_affiliate();
            if (!$up_uid) {
                $up_uid = $other['parent_id'];
            }
            if ($up_uid) {
                $res = $this->row("SELECT * FROM " . $this->pre . "users WHERE user_id = '$up_uid'");
                if ($res['is_proxy']){
                    //设置推荐人
                    $sql = 'UPDATE ' . $this->pre . 'users SET  parent_id = ' . $up_uid . ', parent_proxy_user_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
    ...
    function get_profile($user_id) {
        ...
        $sql = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank," .
                ...
                " nick_name, identity, address, country, province, city " .
...
//}
    function get_business_address() {
        $parent_proxy_user_id = null;
        $address_id = null;
        //找上级的退货地址,如果默认地址存在，否则找第一个
        $sql = 'SELECT u.* ' .
            ' FROM ' . $this->pre . 'users AS u ' .

            " WHERE u.user_id = '$_SESSION[user_id]'";
        if ($row = $this->row($sql)) {
            $parent_proxy_user_id = $row['parent_proxy_user_id'];
            $sql = 'SELECT u.* ' .
                ' FROM ' . $this->pre . 'users AS u ' .

                " WHERE u.user_id = '$parent_proxy_user_id'";
            if ($row = $this->row($sql)) {
        ...
        $address = '';
        if ($parent_proxy_user_id) {
           $consignee = $this->get_default_consignee($parent_proxy_user_id, $address_id);
           if (!$consignee) {
               return $address;
           }
            $sql = "SELECT region_name FROM " . $this->pre .
                "region WHERE region_id = '".$consignee['province']."'";
            $adress = $this->query($sql);
            if ($adress && count($adress)>0) {
                $address .= $adress[0]['region_name'];
            }
            //dump($adress);exit;
            $sql = "SELECT region_name FROM " . $this->pre .
                "region WHERE region_id = '".$consignee['city']."'";
            $adress = $this->query($sql);
            if ($adress && count($adress)>0) {
                $address .= $adress[0]['region_name'];
            }
            $sql = "SELECT region_name FROM " . $this->pre .
                "region WHERE region_id = '".$consignee['district']."'";
            $adress = $this->query($sql);
            if ($adress && count($adress)>0) {
                $address .= $adress[0]['region_name'];
            }
            $address.= $consignee['address'] . '收件人：' . $consignee['consignee'] . '联系电话：' . $consignee['mobile'];
...
//        }
     ...
     * @return  array
     */
    function get_default_consignee($user_id, $id = 0) {
        if ($id) {
            $where['user_id'] = $user_id;
            $where['address_id'] = $id;
            $this->table = 'user_address';
            ...
            $sql = 'select ua.*,u.address_id as adds_id from ' . $this->pre . 'user_address as ua left join '. $this->pre . 'users as u on ua.address_id =u.address_id'. ' where ua.user_id = ' . $user_id . ' order by ua.address_id';
