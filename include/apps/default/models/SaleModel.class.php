...
class SaleModel extends BaseModel {
    ...
    function get_sale_money_total($uid=0){
        ...
        $goods_info =  M()->getRow("select sum(o.goods_amount) as money from {pre}order_info as o join {pre}drp_order_info as d on o.order_id = d.order_id  where o.pay_status='".PS_PAYED."' and d.drp_id = ".$drp_id." and o.user_id != ".$uid);
    ...
    function get_sale_info($key=false){
        ...
        if($key == 'wfk'){
           // $sql = "SELECT user_id FROM {pre}order_info where user_id > 0 and pay_status = ".PS_UNPAYED." and parent_id = " . $_SESSION['user_id'] . " GROUP BY user_id";
            ...
            $sql = "";
            if ($admin) {
                $sql = "SELECT o.user_id FROM {pre}order_info as o LEFT JOIN {pre}users as u on o.user_id = u.user_id  where o.user_id > 0 and o.pay_status = ".PS_UNPAYED." and (u.parent_proxy_user_id = " . $_SESSION['user_id'] . " OR u.parent_proxy_user_id = 0) GROUP BY o.user_id";

            } else {
                $sql = "SELECT o.user_id FROM {pre}order_info as o LEFT JOIN {pre}users as u on o.user_id = u.user_id  where o.user_id > 0 and o.pay_status = ".PS_UNPAYED." and u.parent_proxy_user_id = " . $_SESSION['user_id'] . " GROUP BY o.user_id";
            ...
            $sql = "";
            if ($admin) {
                $sql = "SELECT o.user_id FROM {pre}order_info as o LEFT JOIN {pre}users as u on o.user_id = u.user_id  where o.user_id > 0 and o.pay_status = ".PS_PAYED." and (u.parent_proxy_user_id = " . $_SESSION['user_id'] . " OR u.parent_proxy_user_id = 0) GROUP BY o.user_id";

            } else {
                $sql = "SELECT o.user_id FROM {pre}order_info as o LEFT JOIN {pre}users as u on o.user_id = u.user_id  where o.user_id > 0 and o.pay_status = ".PS_PAYED." and u.parent_proxy_user_id = " . $_SESSION['user_id'] . " GROUP BY o.user_id";
            ...
            $sql = "";
            if ($admin) {
                $sql = "SELECT user_id FROM {pre}users as u join {pre}wechat_user as w on u.user_id=w.ect_uid where u.user_id > 0 and w.subscribe = 1 and u.parent_proxy_user_id = " . $_SESSION['user_id'] . " GROUP BY user_id";
