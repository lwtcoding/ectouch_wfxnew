...
class SaleController extends CommonController {
    ...
    public function order_list(){
        ...
        $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
        //判断是否总代理，总代理能看所有drp为0的
        $sql = "select * from {pre}admin_user where proxy_user_id = $_SESSION[user_id] ";
        $c_admin = $this->model->getRow($sql);
        if ($c_admin && $c_admin['proxy_user']) {
            if ($c_admin['proxy_default']) {
                $where = ' ( d.drp_id = '.$drp_id ." or d.drp_id = 0 ) and o.user_id != $_SESSION[user_id] ";
            } else {
                $where = ' d.drp_id = '.$drp_id ." and o.user_id != $_SESSION[user_id] ";
            }
        } else {
            $where = ' d.drp_id = '.$drp_id ." and o.user_id != $_SESSION[user_id] ";
        }

        $sql = "select count(*) as count from {pre}drp_order_info as d right join {pre}order_info as o on d.order_id=o.order_id where ".$where;
