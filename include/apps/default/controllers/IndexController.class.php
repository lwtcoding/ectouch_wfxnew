...
class IndexController extends CommonController {
    ...
    public function index() {
        ...
        if (empty($_SESSION['user_id'])) {
            //找总代理
            $condition = " proxy_user = 1 and proxy_default = 1 ";
            $total_proxy = $this->model->table('admin_user')->where($condition)->find();
            $condition = " user_id = '$total_proxy[proxy_user_id]' ";
            $drp = $this->model->table('drp_shop')->where($condition)->find();
            header('location: '.url('store/index',['u'=>$drp['user_id'],'drp_id'=>$drp['id']]));
        } else {
            //跳转到所属代理的店铺
            $condition = " user_id = '$_SESSION[user_id]' ";
            $user = $this->model->table('users')->where($condition)->find();
            //判断是否代理
            if ($user['is_proxy']) {
                //进入首页不用跳转
            } else {
                //找父级代理
                $condition = " user_id = '$user[parent_proxy_user_id]' ";
                $p_user = $this->model->table('users')->where($condition)->find();
                $condition = " user_id = '$p_user[user_id]' ";
                $drp = $this->model->table('drp_shop')->where($condition)->find();
                header('location: '.url('store/index',['u'=>$drp['user_id'],'drp_id'=>$drp['id']]));
            }
        ...
        if (!ECTouch::view()->is_cached('index.dwt', $cache_id))
        {
            ...
            //new code Start
            $this->assign('onsale_goods', model('Index')->goods_list('onsale', C('page_size')));
            //new code END
