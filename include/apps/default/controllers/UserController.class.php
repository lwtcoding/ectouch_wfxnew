...
class UserController extends CommonController {
    ...
    public function index() {
		...
		$this->assign('is_proxy',$_SESSION['is_proxy']);
		$this->assign('is_vip',$_SESSION['is_vip']);
    ...
    public function profile() {
        ...
        if (IS_POST) {
            ...
            $other['nick_name'] = I('post.nick_name');
            $other['identity'] = I('post.identity');
            $other['address'] = I('post.address');
            $other['country'] = I('post.country');
            $other['province'] = I('post.province');
            $other['city'] = I('post.city');
     ...
     * 加盟商申请
     */
    public function user_vip_apply() {

        if (IS_POST) {
//如果有待审核的不许重复提交
            $row = $this->model->table('user_vip_apply')

                ->where('status=1 and user_id='.$this->user_id)

                ->find();
            if ($row) {
                show_message(L('wait_apply'), L('add_user_vip_apply_already'), url('index'), 'info');

                exit();
            }
            $apply = array(
                'user_id' => $this->user_id,
                'content' => I('post.apply_content'),
            );


                $this->model->table('user_vip_apply')
                    ->data($apply)
                    ->insert();

                show_message(L('wait_apply'), L('add_user_vip_apply_success'), url('index'), 'info');
        ...
        $user = $this->model->table('users')
            ...
            ->find();
        $this->assign('show_btn',true);
        if ($row || $user['is_vip']) {
            $this->assign('show_btn',false);
        }

        // 页面显示
        $this->assign('title', L('user_vip_apply'));
        $this->display('user_join_in.dwt');
    }
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            if (0 == $enabled_sms) {
                ...
                $other['province'] = I('post.province');
                $other['city'] = I('post.city');
                $other['districts'] = I('post.districts');
                $other['mobile_phone'] = I('post.mobile_phone');
                // 验证码检查
...
//                show_message(sprintf(L('register_success'), $username . $ucdata), array(L('back_up_page'), L('profile_lnk')), [url('index/index'),url('index/index')], 'info');
            } else {
                ECTouch::err()->show(L('sign_up'), url('register'));
            }
            exit();
...
//        $this->assign('back_act', $this->back_act);
        $this->assign('back_act', url('index/index'));
...
//        $adress=model('Users')->get_business_address($goods_info['suppliers_id']);
        $adress=model('Users')->get_business_address();
    ...
    public function upload_file() {
        if ($_REQUEST["name"]) {
            $result = $this->ectouchUpload('', 'service_image');
            if ($result['error'] > 0) {
...
//                    "  收件人： ".$shop_name." 联系电话：".$service_phone;
        $addr_str=model('Users')->get_business_address();
        /*处理退货信息*/
        if (IS_POST) {
