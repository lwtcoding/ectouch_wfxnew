...
elseif ($_REQUEST['act'] == 'proxy_list')
{

    /* 检查权限 */
    admin_priv('order_proxy_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', '代理订单');
//    $smarty->assign('action_link', array('href' => 'order.php?act=order_query', 'text' => $_LANG['03_order_query']));
    $smarty->assign('status_list', $_LANG['cs']);   // 订单状态
    ...
    $smarty->display('order_proxy_list.htm');
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    else
    {
        ...
        if (isset($_REQUEST['is_proxy']) && $_REQUEST['is_proxy'] == 1) {
            $smarty->assign('action_link', array('href' => 'order.php?act=proxy_list&' . list_link_postfix(), 'text' => $_LANG['02_order_list']));
            $smarty->assign('is_proxy',1);
        }else {
            $smarty->assign('action_link', array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['02_order_list']));
            $smarty->assign('is_proxy',0);
        }
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            "FROM " . $ecs->table('order_info') . " AS o " .
                ...
                "LEFT JOIN " . $ecs->table('users') . " AS pu ON u.parent_proxy_user_id = pu.user_id " .
            "WHERE o.user_id > 0 " .
            "AND o.extension_code = '' " . order_query_sql('unprocessed');
    /* 判断管理员是否代理和是否总部代理 不是代理列出所有，总部代理列出下级和parent_proxy_user_id=0的，其他代理列出下级 */

    $c_admin = admin_info();
    if ($c_admin['proxy_user']) {
        if ($c_admin['proxy_default']) {
            $sql .= " AND ( pu.user_id = '$c_admin[proxy_user_id]' OR u.parent_proxy_user_id = 0 ) ";
            $sql .= " AND ( u.user_id != '$c_admin[proxy_user_id]' ) ";
        } else {
            $sql .= " AND ( pu.user_id = '$c_admin[proxy_user_id]' ) ";
        }
    }
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT u.user_id, u.user_name FROM " . $GLOBALS['ecs']->table('users') . " AS u ".
                " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS pu ON u.parent_proxy_user_id = pu.user_id ".
                " WHERE (u.user_id LIKE '%" . mysql_like_quote($id_name) . "%'" .
                " OR u.user_name LIKE '%" . mysql_like_quote($id_name) . "%')" ;
 ...
 * @return void
 */
function order_list($is_proxy = 0)
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        /* 判断管理员是否代理和是否总部代理 不是代理列出所有，总部代理列出下级和parent_proxy_user_id=0的，其他代理列出下级 */
        $c_admin = admin_info();
        if ($c_admin['proxy_user'] && !$is_proxy) {
            if ($c_admin['proxy_default']) {
        ...
        /* 记录总数 */
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o ".
                " LEFT JOIN ". $GLOBALS['ecs']->table('users') . " AS u ON o.user_id = u.user_id " .
                   " LEFT JOIN ".$GLOBALS['ecs']->table('users') . " AS pu ON u.parent_proxy_user_id = pu.user_id " . $where;
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('delivery_order') . " AS do ".
            " LEFT JOIN " . $GLOBALS['ecs']->table("order_info") ." AS o ON o.order_id = do.order_id ".
            " LEFT JOIN " . $GLOBALS['ecs']->table("users") ." AS u ON o.user_id = u.user_id ".
            " LEFT JOIN " . $GLOBALS['ecs']->table("users") ." AS pu ON pu.user_id = u.parent_proxy_user_id ".
            $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT do.* FROM " . $GLOBALS['ecs']->table("delivery_order") . " AS do ".
                " LEFT JOIN " . $GLOBALS['ecs']->table("order_info") ." AS o ON o.order_id = do.order_id ".
                " LEFT JOIN " . $GLOBALS['ecs']->table("users") ." AS u ON o.user_id = u.user_id ".
                " LEFT JOIN " . $GLOBALS['ecs']->table("users") ." AS pu ON pu.user_id = u.parent_proxy_user_id ".
                $where.
