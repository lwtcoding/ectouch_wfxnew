...
elseif (isset($_POST['Submit']))
{
    ...
    /* 取得配置信息 */
    $sql = "SELECT * FROM " . $ecs->table('payment') . " WHERE pay_code = '$_POST[pay_code]' ";
    $pay = $db->getRow($sql);
    $db_pay_config = unserialize($pay['pay_config']);
    $pay_config = array();
    if (isset($_POST['cfg_value']) && is_array($_POST['cfg_value']))
    {
        $upload_index = 0;
        for ($i = 0; $i < count($_POST['cfg_type']); $i++){
            ...
            //是文件，先上传
            if ($_POST['cfg_type'][$i] == 'file') {
                if ($_FILES["cfg_value"]["tmp_name"][$upload_index]) {
                    move_uploaded_file($_FILES["cfg_value"]["tmp_name"][$upload_index], ROOT_PATH."upload/" . $_FILES["cfg_value"]["name"][$upload_index]);
                    $pay_config[$i]['value'] = "upload/" . $_FILES["cfg_value"]["name"][$upload_index];
                }else {
                    //获取数据库数据填充进去
                    $pay_config[$i]['value'] =  $db_pay_config[$i]['value'];
                }
                $upload_index++;
            }
